...
import { ClpCompliantAppHeader } from "@fluidframework/odsp-driver";
import {
    PerformanceEvent,
    isFluidError,
    normalizeError,
...
export class EpochTracker implements IPersistedFileCache {
    ...
    public async get(entry: IEntry): Promise<any> {
        try {
            ...
            if (
                value === undefined ||
                value.version !== persistedCacheValueVersion
            ) {
                return undefined;
            }
            if (value.fluidEpoch === undefined) {
                throw 0x1dc; /* "all entries have to have epoch" */
            }
    ...
    public async validateEpoch(
        ...
        if (epoch === undefined) {
            throw 0x584; /* response should contain epoch */
        }
        try {
        ...
        return this.rateLimiter
            ...
            .catch((error) => {
                ...
                if (
                    ...
                    const redirectLocation = (error as any).redirectLocation;
                    if (redirectLocation !== undefined) {
                        const redirectUrl: IOdspResolvedUrl =
                            patchOdspResolvedUrl(
                                this.fileEntry.resolvedUrl,
                                redirectLocation
                            );
                        const locationRedirectionError =
                            new LocationRedirectionError(
                                error.message,
                                redirectUrl,
        ...
        if (addInBody) {
            ...
            const addHeader = (key: string, val: string) => {
                ...
                if (fetchOptions.headers === undefined) {
                    throw 0x282 /* "Headers should be present now" */;
                }
                (fetchOptions.headers as any)[key] = val;
            };
    ...
    private addParamInBody(
        ...
        if (typeof body !== "string") {
            throw 0x21d /* "body is not string" */;
        }
        const splitBody = body.split("\r\n");
        const firstLine = splitBody.shift();
        if (firstLine?.startsWith("--") !== true) {
            throw 0x21e /* "improper boundary format" */;
        }
    ...
    private async checkForEpochError(
        ...
        if (
            isFluidError(error) &&
            error.errorType === DriverErrorType.fileOverwrittenInStorage
        ) {
...
export class EpochTrackerWithRedemption extends EpochTracker {
    ...
    protected validateEpochFromResponse(
        ...
        super.validateEpochFromResponse(
            epochFromResponse,
            fetchType,
            fromCache
        );
    ...
    public async validateEpochFromPush(details: any) {
        ...
        try {
            ...
            await this._checkForEpochError(error, epoch, "push");
            throw error;
        }
    }
        ...
        await PerformanceEvent.timedExecAsync(
            ...
            async (event) => {
                ...
                const res = await Promise.race([
                    ...
                    this.treesLatestDeferral.promise.finally(() =>
                        clearTimeout(timer)
                    ),
                ]);
                if (res === timeoutRes) {
    ...
    private async _checkForEpochError(
        error: unknown,
        epochFromResponse: string | null | undefined,
        fetchType: FetchTypeInternal,
        fromCache: boolean = false
        ...
        if (
            ...
            const epochError = this._checkForEpochErrorCore(epochFromResponse);
            if (epochError !== undefined) {
                epochError.addTelemetryProperties({
                    fromCache,
                    clientEpoch: this.fluidEpoch,
    ...
    private _checkForEpochErrorCore(
        epochFromResponse: string | null | undefined
    ) {
        // If epoch is undefined, then don't compare it because initially for createNew or TreesLatest
        // initializes this value. Sometimes response does not contain epoch as it is still in
