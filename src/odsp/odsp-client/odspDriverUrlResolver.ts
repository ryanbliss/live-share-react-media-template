...
function getUrlBase(
    ...
    return `${getApiRoot(
        siteOrigin
    )}/drives/${driveId}/items/${itemId}/${version}`;
}
...
export class OdspDriverUrlResolver implements IUrlResolver {
    ...
    public async resolve(request: IRequest): Promise<
        IOdspResolvedUrl & {
    ...
    > {
        if (request.headers?.[DriverHeader.createNew]) {
            ...
            if (createLinkType && createLinkType in ShareLinkTypes) {
                shareLinkInfo = {
                    createLink: {
                        type: (ShareLinkTypes as any)[createLinkType],
                    },
                };
            }
            return {
    ...
    public async getAbsoluteUrl(
        ...
        if (
            relativeUrl === "" &&
            (odspResolvedUrl as any).dataStorePath !== undefined
        ) {
            // If the user has passed an empty dataStorePath, then extract it from the resolved url.
            dataStorePath = (odspResolvedUrl as any).dataStorePath;
        }
