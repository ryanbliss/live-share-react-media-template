...
import { ISnapshotContents } from "@fluidframework/odsp-driver";
    ...
    for (const key of keys) {
        ...
        switch (summaryObject.type) {
            case SummaryType.Tree: {
                treeNode.trees[key] =
                    convertCreateNewSummaryTreeToTreeAndBlobsCore(
                        summaryObject,
                        blobs
                    );
                break;
...
export function convertSummaryIntoContainerSnapshot(
    ...
    if (!isCombinedAppAndProtocolSummary(createNewSummary)) {
        throw new Error(
            "App and protocol summary required for create new path!!"
        );
    }
...
function convertSummaryToSnapshotTreeForCreateNew(
    ...
    for (const key of keys) {
        ...
        switch (summaryObject.type) {
            ...
            case SummaryType.Handle: {
                throw new Error(
                    "No handle should be present for first summary!!"
                );
            }
            default: {
