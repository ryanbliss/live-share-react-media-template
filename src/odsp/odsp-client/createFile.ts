...
function extractShareLinkData(
    requestedSharingLinkKind: ShareLinkTypes | undefined,
    ...
    if (enableSingleRequestForShareLinkWithCreate) {
        ...
        const { sharingLink, sharingLinkErrorReason } = response;
...
export async function createNewEmptyFluidFile(
    ...
    const filePath = newFileInfo.filePath
        ? encodeURIComponent(`/${newFileInfo.filePath}`)
        : "";
    ...
    return getWithRetryForTokenRefresh(async (options: any) => {
        ...
        return PerformanceEvent.timedExecAsync(
            ...
            async (event) => {
                ...
                event.end({
                    headers:
                        Object.keys(headers).length !== 0 ? true : undefined,
...
export async function createNewFluidFileFromSummary(
    ...
    const baseUrl =
        `${getApiRoot(getOrigin(newFileInfo.siteUrl))}/drives/${
            newFileInfo.driveId
        }/items/root:` + `${filePath}/${encodedFilename}`;
    ...
    return createNewFluidContainerCore<ICreateFileResponse>({
        ...
        validateResponseCallback: (content: any) => {
            if (!content?.itemId) {
                throw new NonRetryableError(
