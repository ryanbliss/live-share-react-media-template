...
export class OdspDriver {
    ...
    private static async getDriveIdFromConfig(
        ...
        console.log("OdspDriver:: getDriveIdFromConfig: getting storage token");
        const accessToken = (await this.getStorageToken(
            { siteUrl, refresh: false },
            tokenConfig
        )) as any;
        console.log(
            "OdspDriver:: getDriveIdFromConfig: calling getDriveId for",
            siteUrl
        );
        try {
            const res = await getDriveId(siteUrl, "", undefined, {
